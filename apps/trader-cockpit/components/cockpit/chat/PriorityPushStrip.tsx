        ...
        {pushes.slice(0, 3).map((push) => {
          ...
          return (
            <Button
              ...
              className={
                selected
                  ? "h-auto min-h-[92px] justify-start border-warning/60 bg-warning/10 p-3 text-left shadow-sm shadow-warning/10"
                  : "h-auto min-h-[92px] justify-start border-border bg-background/70 p-3 text-left hover:border-warning/40 hover:bg-surface-secondary"
              }
            >
              <span className="block min-w-0 flex-1">
                <span className="flex min-w-0 items-center justify-between gap-3">
                  <span className="min-w-0 truncate text-[11px] uppercase tracking-wider text-muted">{push.createdAt}</span>
                  <Chip size="sm" className="shrink-0 border border-border bg-transparent uppercase text-muted">
                    {t("chat.agentPush")}
                  </Chip>
                </span>
                <span className="mt-1.5 block line-clamp-1 text-sm leading-5 text-foreground">{push.text}</span>
                <span className="mt-2 flex flex-wrap gap-1.5">
                  {push.tags.slice(0, 2).map((tag) => (
