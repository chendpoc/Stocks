...
# === Stage 1 自进化工作流（T006 — apps/trader-workflows + 薄 CLI）===
npm run trader-agent:backend:dev                   # decide/outcomes 等需 :8000 /api/intel/stage1
cd apps/trader-workflows && npm test               # 47 项 graph/service/runtime 单测
npm run trader-cli -- runs list --json             # 离线可查 checkpoint runs
npm run trader-cli -- decide TSLA.US --json        # DecisionGraph（需后端 + .env LLM）
...
npm run trader-workflows -- runs list --json       # 直连 workflows（不经 CLI 层）
...
│   │   │   │   └── api/              # intel 路由 + stage1（/api/intel/stage1 — T006）
│   │   │   └── tools/                # 外部数据源适配器
...
│   │   │   ├── commands/             # intel 子命令 + Stage1 薄包装（decide/runs/outcomes/eval/insights）
...
│   ├── trader-workflows/             # **Stage 1 LangGraph 运行时**（四图 + checkpoint；独立 npm — T006）
│   │   ├── src/index.ts              # workflows CLI + WorkflowEnvelope
│   │   ├── runtime/                  # stage1Runtime + checkpointStore（与 market_intel 分离）
│   │   ├── graphs/                   # decision / outcome / evaluation / insightExploration
│   │   ├── services/                 # contextSnapshots / decisions / outcomes / evaluation / insightCandidates
│   │   ├── llm/                      # provider + decisionEnvelope
│   │   └── api/client.ts             # → /api/intel/stage1 + POST /context/build
...
| **Stage 1 API** | `intel/api/stage1.py` | `/api/intel/stage1/*` — snapshots、model_decisions、outcomes、insight_candidates、evaluation_reports；后端零 LLM（T006） |

### Stage 1 三应用边界（T006）

| 应用 | 职责 | 禁止 |
|---|---|---|
| `trader-agent/backend` | Stage1 域持久化 + 409 不可变约束 | LangGraph 编排、LLM 调用 |
| `trader-workflows` | checkpoint 运行时 + 四图编排 + JSON envelope | 直接改 `market_intel` 旧表；TUI |
| `trader-cli` | Ink TUI + intel 子命令；Stage1 用 `spawnSync` 调 `trader-workflows` | `import apps/trader-workflows/src/**` |
...
trader (Commander)
  ...
  │    └─ commands/* + services/（intel 路径）
  ├─ decide / runs / outcomes / eval / insights   # Stage1：spawn npm trader-workflows（薄包装）
  │
  └─ llm/
...
13. **Stage1 CLI 薄包装 + Windows shell**（T006）：`decide`/`runs`/… 经 `spawnSync(npm, [--prefix, apps/trader-workflows, run, workflows, …], { shell: win32 })` 转发；**不要**在 `trader-cli` 里 `import` workflows 源码。`runs list` 仅需本地 checkpoint；`decide`/`insights explore` 需 `npm run trader-agent:backend:dev` 与仓库根 `.env` LLM。非零 workflow 退出码原样 `process.exit`。
...
| T006 | `self-evolving-agent-stage1` | Stage 1 自进化闭环（workflows 四图 + stage1 API + 薄 CLI） | in_progress（S1–S8 文档/验证收尾） |

参考：`docs/workflow.md` · `docs/research-agent/target-system/trader-agent/00-workflow-router.md` §5.0 · `.agent-dev/specs/self-evolving-agent-stage1/spec.json`（V208 CLI smoke）。
