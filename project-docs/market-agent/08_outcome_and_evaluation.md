...
6. 生成 outcome_label。
7. 将结果写入 `decision_outcomes` / `insight_candidate_outcomes`（概念名：outcome_memories）。
...
读取尚未被指定窗口回标的 `model_decisions`（概念名：decision_memories）。
...
```bash
npm run workflows -- outcomes run --due --symbol TSLA --window 2h
```
...
## 29. Task 005：OutcomeGraph MVP（复用 / 验收补强）

### 29.1 目标

复用已实现的 `OutcomeGraph`，对历史 `DecisionEnvelope` 进行结果回标，并补齐 Market Agent 所需的表映射、接口接线和验收测试。
...
```text
1. existing OutcomeGraph adapter
2. load_unlabeled_decisions mapping
3. resolve_outcome_window coverage
4. fetch_future_price_window adapter
5. compute_reference_price coverage
6. check_entry_hit coverage
7. check_invalidation_hit coverage
8. compute_mfe_mae coverage
9. assign_outcome_label coverage
10. persist_outcome_memory mapping
```
...
## 30. Task 006：EvaluationGraph MVP（复用 / 验收补强）

### 30.1 目标

复用已实现的 `EvaluationGraph`，对 outcome 结果进行聚合统计并生成 insight_candidate，补齐 Market Agent 所需的 repository 映射、CLI 接线和验收测试。
