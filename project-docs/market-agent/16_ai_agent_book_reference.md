# 16. AI Agent Book Reference — Agent 工程原则与架构参考

> 状态: design | 依赖: `00_README.md`, `02_architecture_overview.md`, `09_pattern_memory_and_learning.md`, `14_llm_reasoning_strategy.md`, `15_ai_quant_book_reference.md`

## 1. 文档目的

本文档沉淀深度阅读 [AI Agent Architecture: From Concept to Production](https://github.com/waylandzhang/ai-agent-book)（Wayland Zhang 著）后的 Agent 工程原则和架构参考，并转化为 Market Agent 系统的具体开发行动。
...
**Market Agent 现状**：
- 系统有 5 个 Graph（Decision / Outcome / Evaluation / InsightExploration / AlphaResearch），编排关系只在代码中隐式存在。
- `02_architecture_overview.md` 列出了模块关系图，但没有标注每个 Graph 的编排模式和状态流转方向。

**缺失的是什么**：
- DecisionGraph 是什么编排模式？（链式 Pipeline：data → quality → feature → setup → evidence → risk → decision）
- Graph 之间如何流转？（Decision → Outcome 是串行触发；InsightExploration 和 AlphaResearch 可并行）
- 每个 Graph 的触发条件、停止条件和失败恢复策略是什么？

**开发行动**：

| 任务 | 位置 | 说明 |
|---|---|---|
| **编排模式标注** | `02_architecture_overview.md` 或新建 `graphs/orchestration-patterns.md` | 为每个 Graph 标注：模式类型（Pipeline / DAG / Loop）、触发条件、依赖、输出、失败策略 |
| **Graph 间流转图** | 同上 | 画出 5 个 Graph 之间的触发关系、数据流向和状态传递 |
| **接口契约文档** | `apps/trader-workflows/src/graphs/` 各目录下的 README | 每个 Graph 的输入 schema、输出 schema、side effects |
...
DecisionGraph
  ...
  触发: CLI `trader scan` 或 MarketMonitorGraph 定时触发
  依赖: 行情数据就绪 + 股票池配置
...
- 但 CLI Chat 页面可调用全部 22 个 Longbridge 工具，`generate_contra` 节点的工具白名单未在文档中定义。
...
- **隔离**：❌ 未设计。Chat 页面和 DecisionGraph 的 LLM 调用共用同一个上下文窗口，可能互相干扰。
...
| **上下文隔离** | `apps/trader-workflows/src/` | Chat 页面的 messages 与 DecisionGraph 的 build_evidence 上下文物理隔离——不共享 session |
...
- Chat session 和 DecisionGraph run 不共享上下文状态
...
**Market Agent 现状**：
- 短期记忆：CLI Chat 页面的 20 轮 messages（隐式实现，无文档标注）
...
| 任务 | 位置 | 说明 |
|---|---|---|
| **Chat 页面 Reflection** | `apps/trader-cli/src/` | 当用户在 Chat 中提问交易问题时，可选启用"先回答 → 自我批判 → 修正"的三步 Reflection 流程 |
...
| **回归测试基线** | `apps/trader-workflows/tests/` | 保存一批已验证的 DecisionEnvelope 作为 golden dataset，修改 RiskGate/FeatureEngine 后自动回归 |
...
| **P2P 消息** | `Mailbox` + 5 种协议（Request/Offer/Accept/Delegation/Info） | Agent 间协商、竞标 | 高 |
...
| 书中层次 | 我们有没有 | 实现方式 | 缺口 |
|---|---|---|---|
| **依赖注入** | ✅ 有 | Graph 内部：LangGraph state 传递；Graph 间：DB 表（`model_decisions` → `decision_outcomes`） | 缺少 Plan IO 声明——不知谁消费谁的数据 |
| **工作空间** | ⚠️ 隐式有 | `session_context_packs` 全量加载 PatternMemory → 注入 CLI Chat/Dashboard 上下文 | 无 Topic 语义、无增量读取、无 TTL 清理 |
| **P2P 消息** | ❌ 没有 | — | 当前不需要（Graph 间无需协商） |

核心缺口：**Plan IO 声明**——当前 Graph 之间的数据依赖关系完全依赖 DB 表名推断，没有显式的 Produces/Consumes 文档。新开发者无法快速理解数据流。

**开发行动**：

| 任务 | 位置 | 优先级 | 说明 |
|---|---|---|---|
| **Plan IO 声明** | 3.1 编排模式标注中的 Graph 模板 | P0 | 每个 Graph 显式声明 Produces/Consumes（topic 语义） |
...
**验收标准**：
- 5 个 Graph 均有 Produces/Consumes 声明（在 3.1 编排模式标注中完成）
...
| Swarm 特征 | 我们系统 | 判定 |
|---|---|---|
| Lead Agent 事件驱动编排 | 预定义的 Graph 触发器（CLI 命令 / 定时器） | ❌ 不需要 — 我们的 Graph 数量和触发规则固定 |
| 动态 spawn Worker | Graph 是静态注册的，不能运行时动态创建 | ❌ 不需要 — 固定 8 只标的 + 5 类 setup，分析范围确定 |
| idle → reassign | Graph 执行完就退出，不能复用 | ❌ 不需要 — Graph 功能确定，OutcomeGraph 不能被 reassign 去跑 DecisionGraph |
| human_input 事件 | CLI Chat 页面可以在运行前后介入，不能中途干预 | ⚠️ 中期可吸收 — 用户如果在 DecisionGraph 运行中能实时插入提示（如"不要参考昨天的新闻"）会提升体验 |
| 收敛检测 | 无显式检测机制 | ❌ 不需要 — ReAct 循环 max 3 steps 已限定迭代次数 |

**核心判断**：Swarm 解决的是"任务不确定性高、需要动态调整"的编排问题。我们当前系统"确定性 workflow 优先"的设计哲学与 Swarm 的 Lead Agent 动态路由有本质差异。对于 MVP（固定标的池 + 固定 setup + 固定 Graph 触发规则），静态的 DAG/Pipeline 编排完全足够。

**中期可吸收的元素**：

| 元素 | 何时需要 | 落地方式 |
|---|---|---|
| **human_input 事件** | 当用户希望在 Graph 运行中途插入提示时 | 在 `Stage1Runtime` 中加入 `human_input` 事件通道，Graph 节点在关键 checkpoints（如 build_evidence 完成后）暂停等待 |
...
#### 3.9 Planning 模式 — 前置规划能力

**书中设计**（Part 4 第 10 章）：Planning 模式让 Agent 在行动前先制定计划——分解任务、评估依赖、分配步骤。

**对我们的启发**：当前 `trader scan` 是一次性扫描，没有"分步执行"的概念。未来如果引入多标的顺序扫描、分批处理，可能需要 Planning 模式。

**判定**：当前单次扫描的用户场景不需要 Planning。待多标的自动监控上线后再评估。

---

#### 3.10 Token Budget — 成本控制

**书中设计**（Part 8 第 24 章）：企业级 Agent 系统需要 Token 预算管理——为每次 LLM 调用设定 Token 上限、追踪累计使用量、设置日/周预算上限。

**开发行动**：`apps/trader-workflows/src/` 中为每个 LLM 调用点加入 Token 计数和日志。远期可加入日预算上限（如每天不超过 100K tokens）。
...
- **Tracing**：跨 Graph 的完整调用链追踪

**Market Agent 现状**：现有 `Stage1Runtime` 已有 run/checkpoint/audit 记录（✅ Logging）。缺少 Metrics 和 Tracing。

**开发行动**：在 `trader-workflows` 的 Runtime 中加入：
- 每个 Graph 节点的执行延迟（p50/p95/p99）
- 每次 LLM 调用的 Token 消耗
- 跨 Graph 的 run_id 关联
...
```text
Phase 1（当前迭代，P0）：
  1. 编排模式显式化 — 标注每个 Graph 的模式和流转 (3.1)
...
P0 三项可与 15 号文档的 P0 项（Regime Detection、Risk Gate 强化、Setup 衰减监控）并行推进——两者操作不同的代码模块，互不阻塞。
...
| 参考点 | 新增/修改的模块 | 不影响 |
|---|---|---|
| 编排模式显式化 + Plan IO | `02_architecture_overview.md`（改）、各 Graph 目录 README（新） | Graph 代码不变 |
...
| Handoff Plan IO + Workspace | `session_context_packs`（改）、DB 表文档注释 | Graph 间流转逻辑不变 |
| Swarm human_input | `Stage1Runtime`（改） | 现有 Graph 节点逻辑不变 |

---

## 7. 风险与边界

- **编排模式文档化不改代码**：编排模式标签是文档层面的描述，不改变任何 Graph 的执行逻辑。避免为了"标准化"而重构已有工作流。
- **工具白名单收紧需谨慎**：当前 Chat 页面可调用全部 Longbridge 工具，收紧白名单可能影响用户交互体验。建议先文档化矩阵，再逐节点收紧，最后才考虑 Chat 页面。
- **上下文隔离不能破坏 Chat 体验**：DecisionGraph 和 Chat 保持上下文隔离，但 Chat 页面需要能读取当前 run 的结果（通过 DB，而非共享上下文）。
...
完成 Phase 1 后：
- [ ] 每个 Graph 有明确的编排模式标注（含 Produces/Consumes）
- [ ] Graph 间流转图清晰可读
- [ ] `14_llm_reasoning_strategy.md` 包含完整的工具白名单矩阵和风险等级
- [ ] `build_evidence` 注入的每条数据源有长度上限
- [ ] Chat session 和 DecisionGraph run 上下文物理隔离
