...
| 7 | 记忆、归档路线、历史 module docs | 背景线索；漂移时必须验证 |
...
旧 `project-docs/archive/**` 只能作为历史材料，不能压过本目录目标系统文档。
...
| 已通过规格门禁的 workflow / CLI 实现 | TDD 或 `agent-module-development-loop` | code diff, tests, verification | only if scope/contract changes |
...
- UI/UX review：当前无 active Web UI 路线；涉及 CLI/TUI 体验时先确认产品边界，reviewer 不得替用户决定交互偏好。
