...
实际实现路由前缀为 `/cockpit/*`（`apps/trader-cockpit/app/cockpit/`）。进展见 [02-web-agent-cockpit-development/00-implementation-status.md](./02-web-agent-cockpit-development/00-implementation-status.md)。

| Route | First-version role | Status |
|---|---|---|
| `/cockpit/dashboard/live` | 市场意图首页：固定股票池、市场状态、机会摘要、图表证据、Agent 状态 | done |
| `/cockpit/signals` | 机会与关注计划中心：signal 状态、触发条件、失效条件、情景树、证据 | done |
| `/cockpit/chat` | Agent Console：只读上下文、workstream、活动 trace、节点详情 | done (0D-1) |
| `/cockpit/inbox` | Agent 主动提醒：机会、market gate、风险/失效、学习摘要 | done |
| `/cockpit/playbook-theories` | 交易规律库：PlaybookTheory 和 PlaybookRule 数组 | done |
| `/cockpit/learning` | 有信息量时展示新规律、低置信候选、自我反思 | done |
| `/cockpit/settings` | 轻量偏好和 Tool Settings | done |

第一版不提供旧版控制台管理类 route。工具设置并入 `/settings`；规则与规律库统一为 `/playbook-theories`。

## 4. 数据接入策略

当前实现（2026-05-27）：**mock-first**（`mockCockpitAdapter`）。Phase 1 目标为真实只读 Agent Core 接入 + mock fallback。

```text
真实只读 Agent Core 接入 + mock fallback  ← Phase 1 目标
mock-only adapter                         ← Phase 0 当前
```
