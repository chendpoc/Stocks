...
本文档包是 `01-agent-core-backend-prd.md` 的实施拆解层。它只服务 `project-docs/research-agent/target-system/trader-agent/` 目标系统，不继承旧 `project-docs/research-agent/modules/` 路线。
...
```powershell
rg --files project-docs/research-agent/target-system/trader-agent/01-agent-core-development
pnpm run docs:build
```
