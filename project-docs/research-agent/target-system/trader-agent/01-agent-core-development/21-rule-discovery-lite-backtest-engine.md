...
## Candidate Family Taxonomy

Candidate family is a finite classification used by Rule Discovery and Insight Exploration to constrain alpha research into auditable buckets. It is not a storage system, plugin registry, strategy activation mechanism, or proof that a candidate works.

Source of truth in code:

- `apps/trader-workflows/src/services/candidateFamilies.ts`

V0 families:

| Family | Role | What it asks | Typical examples |
|---|---|---|---|
| `momentum_trend` | Primary research family | Do prior winners, breakouts, or relative strength continue? | 3-12 month momentum, short-term breakout, sector-relative strength |
| `mean_reversion` | Primary research family | Did the market overreact, and is a partial reversion likely? | gap fill, sharp-drop rebound, open-drive fade, VWAP reversion |
| `event_driven` | Primary research family | Does the market underreact or overreact after a dated event? | earnings drift, filing reaction lag, holder reduction digestion, regulatory-news repricing |
| `liquidity_flow_microstructure` | Primary research family | Do flow, volume, or intraday market structure predict price moves? | fixed-time intraday rally/selloff, volume shock, VWAP reclaim, close pressure |
| `relative_value_lead_lag` | Primary research family | Does a related asset, sector, index, or spread lead the target? | pair divergence, sector-neutral spread, index-to-component lead-lag, supplier-customer lag |
| `cross_sectional_filter` | Supporting filter | Which symbols should be included, excluded, or risk-adjusted? | quality screen, earnings-yield screen, low-beta filter, liquidity eligibility filter |

Usage rules:

- Every `RuleCandidate` or `InsightCandidate` that uses this taxonomy must choose one known family id.
- A family only describes the research shape. It does not replace trigger, entry condition, exit condition, invalidation, evidence requirements, or lite backtest.
- `cross_sectional_filter` is supporting context in v0. It may restrict universe or risk, but should not become a standalone trading rule unless a later design explicitly promotes that behavior.
- New families should be added only when repeated candidates cannot be cleanly expressed by the current six buckets.
